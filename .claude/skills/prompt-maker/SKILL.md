...
| **Constraints** | What should and should not happen, scope boundaries — frame as positive instructions with motivation where possible | If complex |
...
- XML tags help for complex multi-section prompts: `<context>`, `<task>`, `<constraints>`, `<output_format>`. Markdown inside XML tags is fine — XML defines section boundaries, markdown formats content within them.
- Tell Claude what to do instead of what not to do. "Your response should be smoothly flowing prose paragraphs" outperforms "Do not use markdown."
- Provide context and reasoning WHY, not just WHAT — Claude generalizes better from explanations. Adding motivation ("the database connection is read-only, so DML will fail") is more effective than bare rules ("Do not write DML").
- Claude 4.6 is highly responsive to instructions — dial back aggressive language. Where older models needed "CRITICAL: You MUST use this tool when...", Claude 4.6 works better with "Use this tool when...". Over-prompting causes overtriggering.
...
- For long-context prompts (20k+ tokens): place longform data at the top, query and instructions at the bottom. This can improve response quality by up to 30%.
- Always specify output format and length explicitly
- For grounding: "Ground every claim in data you retrieved — do not fabricate or extrapolate" with motivation ("the output feeds downstream audit decisions") outperforms bare "Do not hallucinate."
...
- Stop conditions are important — runaway loops waste credits. But dial back aggressive language: Claude 4.6 is more responsive and triggers tools readily without over-prompting.
- Claude Opus 4.x over-engineers — add "Only make changes directly requested. Do not add extra files, abstractions, or features."
- Scope to specific files and directories — avoid global instructions without a path anchor
- Human review triggers recommended: "Stop and ask before deleting any file, adding any dependency, or affecting the database schema"
- If previous prompts encouraged aggressive tool use ("If in doubt, use [tool]"), remove that language — Claude 4.6 triggers tools appropriately without it, and over-prompting causes overtriggering.
- Context awareness: Claude 4.6 tracks remaining context window. For long tasks, add: "Your context will be compacted as it approaches its limit. Save progress before context refreshes."
...
**Framing failures**
- Negative constraint without positive alternative → reframe: "Do not use markdown" becomes "Write in smoothly flowing prose paragraphs"
- Bare rule without motivation → add context: "Only SELECT statements" becomes "Only SELECT statements — the connection is read-only, so DML will fail"
- Over-aggressive signal words for capable models (Claude 4.6, GPT-5.x) → soften to natural language. Reserve ALL-CAPS and "MUST/NEVER" for less capable models or genuine safety constraints.
...
**Chain of Thought** — for logic, math, and debugging on standard reasoning models ONLY (Claude without adaptive thinking, GPT-5.x, Gemini, Qwen2.5, Llama). Never on o3/o4-mini/R1/Qwen3-thinking.
"Think through this step by step before answering."
Note: Claude 4.6 with adaptive thinking decides when and how deeply to reason internally. Prefer general instructions ("think thoroughly") over prescriptive step-by-step plans — Claude's reasoning frequently exceeds what a human would prescribe. If thinking is off, manual CoT with `<thinking>` and `<answer>` tags still works.
...
3. Is each constraint framed as what to do (not what to avoid), with motivation where non-obvious? For Claude 4.6: use natural language over aggressive capitalization. For less capable models (Llama, Mistral, older GPTs): firmer signal words are still appropriate.
